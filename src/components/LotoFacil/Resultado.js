...
const Resultado = () => {
  ...
  async function buscarResultado() {
    ...
    setError(null);
    try {
      const response = await fetch(`https://servicebus2.caixa.gov.br/portaldeloterias/api/lotofacil${sorteioInput}`);
      if (!response.ok) {
  ...
  return (
    <div className="container Resultado">
      <div className="card resultadoLotoFacil" >
        ...
        {!loading && !error && resultado && (
          <div className="content-result">
            ...
            <span style={{ color: '#931d89' }} className="item" id="numero">{resultado.numero}</span>

            <p className="resultText" style={{ color: '#585858' }}>Data de Apuração:</p>
            <span style={{ color: '#931d89' }} className="item" id="dataApuracao">{resultado.dataApuracao}</span>

            <p className="resultText" style={{ color: '#585858' }}>Números Sorteados:</p>
            <span style={{ color: '#931d89' }} className="item dezenasContainer" id="listaDezenas">
