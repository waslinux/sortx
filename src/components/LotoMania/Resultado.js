...
const Resultado = () => {
  ...
  function fetchData() {
    setLoading(true);
    setError(null);
    fetch('https://servicebus2.caixa.gov.br/portaldeloterias/api/lotomania')
      .then((response) => {
        if (!response.ok) {
  ...
  async function buscarResultado() {
    ...
    setError(null);
    try {
      const response = await fetch(`https://servicebus2.caixa.gov.br/portaldeloterias/api/lotomania/${sorteioInput}`);
      if (!response.ok) {
  ...
  return (
    <div className="container Resultado">
      <div className="card resultadoLotoMania" >
        ...
        {!loading && !error && resultado && (
          <div className="content-result">
            ...
            <span style={{color:'#F78100'}} className="item" id="numero">{resultado.numero}</span>

            <p className="resultText" style={{ color: '#585858' }}>Data de Apuração:</p>
            <span style={{color:'#F78100'}}  className="item" id="dataApuracao">{resultado.dataApuracao}</span>

            <p className="resultText" style={{ color: '#585858' }}>Números Sorteados:</p>
            <span style={{color:'#F78100'}}  className="item dezenasContainer" id="listaDezenas">
